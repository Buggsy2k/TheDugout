    ...
    <div className="page">
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card">
          ...
          <div className="stat-content">
            <span className="stat-label">Est. Value (Low)</span>
            <span className="stat-value">{formatCurrency(stats.totalValueLow)}</span>
          </div>
        </div>
        <div className="stat-card">
          <div className="stat-icon"><TrendingUp size={24} /></div>
          <div className="stat-content">
            <span className="stat-label">Est. Value (High)</span>
            <span className="stat-value">{formatCurrency(stats.totalValueHigh)}</span>
          </div>
        </div>
        <div className="stat-card">
      ...
      {stats.topValueCards.length > 0 && (
        <section className="dashboard-section">
          <h2>Top 10 Most Valuable Cards</h2>
          <div className="card-grid">
            {stats.topValueCards.map(card => (
      ...
      {stats.totalCards === 0 && (
        <div className="empty-state">
          ...
          <p>Your collection is empty. Start by adding cards individually or use bulk entry to photograph a binder page.</p>
          <div className="empty-actions">
            <Link to="/cards/new" className="btn btn-primary">Add a Card</Link>
            <Link to="/bulk-entry" className="btn btn-secondary">Bulk Entry</Link>
          </div>
        </div>
      )}
    </div>
